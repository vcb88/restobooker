...
1.  Создайте файл `.env` на основе примера:
    ...
    cp .env.example .env
    ```
2.  Отредактируйте `.env`, добавив свои API ключи.
...
4.  **Важно:** При первом запуске необходимо авторизоваться в Telegram. Для этого подключитесь к контейнеру:
    ```bash
    docker attach iron_business_hostess
    ```
    Введите номер телефона и код подтверждения. После успешного входа можно отсоединиться от контейнера (Ctrl+P, Ctrl+Q) или оставить его работать в фоне.
